<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"170rpx","fontSize":"32rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}'>类型：</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','shipinleixing');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','shipinleixing');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('shipinleixing', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussshipinleixing/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('shipinleixing', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('shipinleixing', this.detail);
				this.$utils.msg('审核成功');
